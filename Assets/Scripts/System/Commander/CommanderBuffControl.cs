...
public class CommanderBuffControl
{
...
//    public event RateHealthCalculateDelegate rateHealthCalculate;
    ...
    public enum TYPE_CMD_ABILITY { MunitionLv, MunitionInc, MunitionGet, UnitCard, HeroCard, SkillCard}
...
//    int preMaxHealth;
    ...
    {
        return m_buffActorList.ToArray<BuffActor>();
    }
    ...
    {
        BuffActor[] buffActorArray = m_buffActorList.ToArray();
    ...
    public bool addBuff(BuffActor buffActor)
    {
        ...
        //같은 키 버프 찾기
        BuffActor useBuffActor = m_buffActorList.Where(buffAct => buffAct.key == buffActor.key).SingleOrDefault();
...
//            useBuffActor.replaceBuff();
        ...
        else
        {
            ...
            //파티클 유닛 하위에 삽입하기
            //EffectActor effectActor = effectActorManager.createActor(buffActor.key + "BuffParticle", unitActor.transform.position, unitActor.typeController);
...
//        preMaxHealth = rateHealthCalculate(preMaxHealth);
    ...
    {
        //같은 버프키가 있으면 가져오기
        return m_buffActorList.Where(buffAct => buffAct.key == buffActor.key).SingleOrDefault();
    }
    ...
    {
        //등록되어있는 버프 삭제
        if (m_buffActorList.Contains(buffActor))
        {
            ...
            //파티클 제거
//            if (m_buffActorDic != null)
//            {
//                m_buffActorDic.Remove(buffActor);
////                buffActor.transform.SetParent(null);
//            }
            ...
            //제거가 성공되면
            if (m_buffActorList.Remove(buffActor))
            {
                //                buffActor.clear();
//                preMaxHealth = rateHealthCalculate(preMaxHealth);
                return true;
